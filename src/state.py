...
REQUIRED_HANDLERS        = {'on_clear':(lambda machine: None),'on_enter':None,'on_exit':None,'on_free':(lambda machine: None),'on_init':(lambda machine: None),'on_update':None};

# addGenericHandler
# adds a handler to the required handler list. If fn is defined, the handler will default to it
def addGenericHandler( name, fn=None ) :
    REQUIRED_HANDLERS.setdefault( name, fn );
    
# bind
# binds a class to a state class. Please don't use this to combine non-state classes
def bind( basestate, baseother=None ) :
    ...
    class bound_state (basestate, baseother) :
        def __init__( self, otherparams=(), stateparams=() ) :
            if not isinstance( otherparams, tuple ) or not isinstance( stateparams, tuple ) :
...
class State :
    ...
    def _config( self, machine ) :
        if self.machine and self.machine == machine :
            ...
            warnings.warn( "detected the state {} is being used in two machines at once. This could corrupt the backtrack list.".format( self ), ResourceWarning, stacklevel=4 );
