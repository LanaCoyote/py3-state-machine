...
class Machine :
  ...
  def _handleCall( self, func, params=() ) :
    ...
    # generic state machines cannot use arbitrary handlers
  ...
  def backtrack( self ) :
    ...
    # go back and then pull up the next state in the backtrack list
  ...
  def changeState( self, new_state, backtrack=False, force=False ) :
    # if we're passed a None state, free ourselves instead
    if new_state == None :
      self.free();
      return;
    ...
    if not isinstance( new_state, self.state_type ) and not self.generic :
      raise TypeError( "changeState expects type {}, not {}".format( 
        self.state_type, 
        type( new_state ) ) );
  ...
  # free
  # clears the machine, dumping all states and clearing all lists
  def free( self ) :
    self.clearHistory();
    if self.state :
      self.state._free();
  ...
  # __eq__
  # check to see if a state is running in the machine
  ...
  # __ne__
  # check to see if a state is not running in the machine
